...
group_by(admin2Name, year) %>% mutate(peak_ndvi = max(newNDVI, na.rm = TRUE)) %>% distinct(admin2Name, year, peak_ndvi, newNDVI) %>%
ungroup()
...
Max NDVI Admin 2 and Admin 3
```{r}
ggplot(nigerShpMerged_admin2) + theme_classic() +
  geom_sf(aes(fill = newNDVI.y), color = NA, alpha = 0.8) +
  scale_fill_viridis_c( direction = -1) +
  labs(title="Annual NDVI by Department (Admin 2)", fill = "NDVI" ) +
  facet_wrap(~year.y)

ggplot(nigerShpMerged_admin3) + theme_classic() +
  geom_sf(aes(fill = newNDVI.y),color = NA, alpha = 0.8) +
  scale_fill_viridis_c( direction = -1) +
...
```{r}
#MaxEVI2017 <- nigerShpMerged_admin2 #%>%
  #filter(District == "Hwange") %>%
  ggplot(nigerShpMerged_admin2, aes(x = year.y, y = peak_ndvi.y)) +
