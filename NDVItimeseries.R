...
ndviData2 <- 
  ...
  select(admin1Name, admin1Pcod, 
         admin2Name, admin2Pcod, "19810701": "20191231") 
...
peakndvi %>% 
  ...
  geom_line() + 
  labs(title="Peak NDVI by Year", x="Year", y ="Peak NDVI") +
  theme_classic() 
...
yearData_admin2 <- 
  ...
  mutate(peak_ndvi = max(newNDVI, na.rm = TRUE),
         mean_ndvi = mean(peak_ndvi, na.rm = TRUE),
         standardDeviation_ndvi = sd(peak_ndvi, na.rm = TRUE)) %>%
  distinct(admin1Name, admin1Pcod,admin2Name, admin2Pcod, year, 
           peak_ndvi, mean_ndvi, standardDeviation_ndvi) %>%
  ungroup()
